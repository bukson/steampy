...
from steampy.models import Asset, SteamUrl, GameOptions, TradeOfferState
from steampy.exceptions import ApiException, LoginRequired, SevenDaysHoldException
...
class SteamClientManager:
    __slots__ = ("api_key", "username", "password", "steam_guard_path", "_client")

    def __init__(
        self, api_key: str, username: str, password: str, steam_guard_path: str
    ) -> None:
        self.api_key = api_key
        self.username = username
        self.password = password
        self.steam_guard_path = steam_guard_path
        self._client: Optional[SteamClient] = None

    def __enter__(self):
        self._client = SteamClient(self.api_key)
        return self._client.login(self.username, self.password, self.steam_guard_path)
...
class SteamClient:
    ...
    def _get_session_id(self) -> str:
        return self._session.cookies.get_dict()["sessionid"]  # type: ignore
    ...
    def make_offer(
        ...
        response = self._session.post(url, json=params, headers=headers).json()
