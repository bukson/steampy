...
class ConfirmationExecutor:
    ...
    def send_trade_allow_request(self) -> dict:
        ...
        params['op'] = Tag.ALLOW.value,
        params['cid'] = confirmation.data_confid
        params['ck'] = confirmation.data_key
        return self._session.get(self.CONF_URL + '/ajaxop', params=params).json()
    ...
    def _get_confirmations(self) -> List[Confirmation]:
        ...
        if soup.select("#mobileconf_empty") is not None:
    ...
    def _fetch_confirmations_page(self) -> requests.Response:
        tag = Tag.CONF.value
        params = self._create_confirmation_params(tag)
        return self._session.get(self.CONF_URL + '/conf', params=params)

    def _fetch_confirmation_details_page(self, confirmation: Confirmation) -> str:
        tag = 'details' + confirmation.id
        params = self._create_confirmation_params(tag)
        return self._session.get(self.CONF_URL + '/details', params=params).json()['html']

    def _create_confirmation_params(self, tag: str) -> dict:
        timestamp = time.time()
        confirmation_key = guard.generate_confirmation_key(self._identity_secret, timestamp)
    ...
    def _get_confirmation_trade_offer_id(confirmation_details_page: str) -> str:
        ...
        full_offer_id = soup.select(".tradeoffer")['id']
